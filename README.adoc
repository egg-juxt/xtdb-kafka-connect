...
`patch`:: An upsert by id. Fields with a NULL value are kept unchanged, rather than set to NULL.
...
`record_value`::
(default) The `_id` field of the record value will be used

`record_key`::
(required if you want https://kafka.apache.org/documentation/#design_compactionbasics[tombstones] to delete records) The record key must be either a Struct or a primitive value. If the key is a struct its `_id` field will be used.
...
For XTDB logging configuration, refer to the XTDB documentation on https://docs.xtdb.com/ops/troubleshooting.html[troubleshooting].
